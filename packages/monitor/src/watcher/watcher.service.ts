...
export class MonitorZKLAmountService implements OnModuleInit {
  ...
  public constructor(
    ...
    this.monitorAddressList = configService.get<IMonitorAddress[]>(
      'monitor.monitorAddressList',
    );
  ...
  async monitorInit() {
    ...
    if (blockNumberList.length === 0) {
      ...
      [initNovaData, initNovaBlock] = await Promise.all([
        this.balanceRepository.findAllAddressAmount(
          novaAddressList,
          this.zklNovaAddress,
        ),
    ...
    if (lastZKLAmountDataList.length === 0) {
      const records = [];
      for (let i = 0; i < this.monitorAddressList.length; i++) {
        const amount = await this.getTodayZKLAmountByAddress(
          this.monitorAddressList[i].Address,
          this.zklNovaAddress,
          this.zklEthAddress,
          this.monitorAddressList[i].Network,
          this.monitorAddressList[i].Owner,
          initETHData,
          initNovaData,
        );
        const time = new Date();
        const timeStr =
          ...
          time.getDate();
        const preAmount =
          await this.monitAddressHistoryRepository.findYesterdayLastZKLAmount(
            this.monitorAddressList[i].Address,
            this.monitorAddressList[i].Network,
            this.monitorAddressList[i].Owner,
            timeStr,
  ...
  private async getTodayZKLAmountByAddress(
    address: string,
    tokenNovaAddress: string,
  ...
  private async monitorEthZklTransfer() {
    ...
    const ethZklTransfer = await this.getOtherChainZKLTransfers(
      this.zklEthAddress,
      ethLookBlockNumber.toString(),
      ethNowBlockNumber.toString(),
      'Ethereum',
    );
    ethZklTransfer.reverse();
    for (const ethZklTransferElement of ethZklTransfer) {
      //from
      const monitAddressConfig =
        await this.monitAddressConfigListRepository.findOneBy({
      ...
      if (monitAddressConfig) {
        ...
        await this.monitAddressLastRepository.updataAmount(
          monitorLast.address,
          monitorLast.owner,
          monitorLast.network,
          monitorLast.zklAmount.sub(
            BigNumber.from(ethZklTransferElement.value),
          ),
          monitorLast.change.sub(BigNumber.from(ethZklTransferElement.value)),
        );
      }
      const monitAddressUser =
        await this.monitAddressUserListRepository.findOneBy({
          address: ethZklTransferElement.from_address,
          network: 'Ethereum',
        });
      if (monitAddressUser) {
        await this.monitAddressUserListRepository.updataAmount(
          monitAddressUser.address,
          monitAddressUser.owner,
          monitAddressUser.network,
          monitAddressUser.zklAmount.sub(
            BigNumber.from(ethZklTransferElement.value),
          ),
        );
        const monitorLastCEXTotalUser =
          await this.monitAddressLastRepository.findOneBy({
            owner: `Total_${monitAddressUser.network}_${monitAddressUser.owner}`,
          });
        await this.monitAddressLastRepository.updataAmount(
          monitorLastCEXTotalUser.address,
          monitorLastCEXTotalUser.owner,
          monitorLastCEXTotalUser.network,
          monitorLastCEXTotalUser.zklAmount.sub(
            BigNumber.from(ethZklTransferElement.value),
          ),
          monitorLastCEXTotalUser.change.sub(
            BigNumber.from(ethZklTransferElement.value),
          ),
        );
      }
      ...
      if (monitAddressConfigForTo) {
        ...
        await this.monitAddressLastRepository.updataAmount(
          ...
          monitorLast.owner,
          monitorLast.network,
          monitorLast.zklAmount.add(
            BigNumber.from(ethZklTransferElement.value),
          ),
          monitorLast.change.add(BigNumber.from(ethZklTransferElement.value)),
        );
      }
      const monitAddressUserForTo =
        await this.monitAddressUserListRepository.findOneBy({
          address: ethZklTransferElement.to_address,
          network: 'Ethereum',
        });
      if (monitAddressUserForTo) {
        await this.monitAddressUserListRepository.updataAmount(
          monitAddressUserForTo.address,
          monitAddressUserForTo.owner,
          monitAddressUserForTo.network,
          monitAddressUserForTo.zklAmount.add(
            BigNumber.from(ethZklTransferElement.value),
          ),
        );
        const monitorLastCEXTotalUser =
        ...
        await this.monitAddressLastRepository.updataAmount(
          ...
          monitorLastCEXTotalUser.owner,
          monitorLastCEXTotalUser.network,
          monitorLastCEXTotalUser.zklAmount.add(
            BigNumber.from(ethZklTransferElement.value),
          ),
          monitorLastCEXTotalUser.change.add(
            BigNumber.from(ethZklTransferElement.value),
          ),
        );
      }
      ...
      if (
        monitAddressConfigForTo &&
        monitAddressConfigForTo.type === 'CEX Wallet'
      ) {
        const zklAmount = await this.getTokenOtherChainZKLAmount(
          ethZklTransferElement.from_address,
          this.zklEthAddress,
          'Ethereum',
        );
        const insertMonitorAddressUser =
          await this.monitAddressUserListRepository.findOneBy({
            address: ethZklTransferElement.from_address,
          });
        if (!insertMonitorAddressUser) {
          ...
          await this.monitAddressLastRepository.updataAmount(
            ...
            monitorLastCEXTotalUser.owner,
            monitorLastCEXTotalUser.network,
            monitorLastCEXTotalUser.zklAmount.add(BigNumber.from(zklAmount)),
            monitorLastCEXTotalUser.change.add(BigNumber.from(zklAmount)),
          );
        }
      }
      ...
      if (transferAmount.gte(zKLValue)) {
        ...
        await this.sendMessageToLarkGroup(
          from,
          to,
          'Ethereum',
          ethZklTransferElement.value,
          ethZklTransferElement.transaction_hash,
        );
      }
    }

    //record blockNumber
    await this.monitorChainRecordRepository.updataBlockNumber(
      this.blockNumberRecordETHName,
      ethNowBlockNumber,
    );
  }
  ...
  private async monitorNovaZklTransfer() {
    ...
    const novaZklTransfer = await this.transferRepository.findAll(
      this.zklNovaAddress,
      novaLookBlockNumber.toString(),
      novaNowBlockNumber.toString(),
    );
    for (const novaZklTransferElement of novaZklTransfer) {
      //from
      const fromAddress =
        '0x' + novaZklTransferElement.transfers_from.toString('hex');
      const monitAddressConfig =
        await this.monitAddressConfigListRepository.findOneBy({
          address: fromAddress,
        });
      if (monitAddressConfig) {
        ...
        await this.monitAddressLastRepository.updataAmount(
          ...
          monitorLast.network,
          monitorLast.zklAmount.sub(
            BigNumber.from(novaZklTransferElement.transfers_amount),
          ),
          monitorLast.change.sub(
            BigNumber.from(novaZklTransferElement.transfers_amount),
          ),
        );
      }
      ...
      //to
      const toAddress =
        '0x' + novaZklTransferElement.transfers_to.toString('hex');
      const monitAddressConfigForTo =
        await this.monitAddressConfigListRepository.findOneBy({
          address: toAddress,
        });
      ...
      if (
        ...
        if (!insertMonitorAddressUser) {
          const zklAmount = await this.balanceRepository.findOne(
            fromAddress,
            this.zklNovaAddress,
          );
          await this.monitAddressUserListRepository.add({
      ...
      const transferAmount = BigNumber.from(
        novaZklTransferElement.transfers_amount,
      );
      const zKLValue = BigNumber.from(monitorZKLValue);
      if (transferAmount.gte(zKLValue)) {
        ...
        const txHash =
          '0x' +
          novaZklTransferElement.transfers_transactionHash.toString('hex');
        await this.sendMessageToLarkGroup(
          from,
          to,
          'zkLink Nova',
    ...
    //record blockNumber
    await this.monitorChainRecordRepository.updataBlockNumber(
      this.blockNumberRecordNovaName,
      novaNowBlockNumber,
    );
  }

  private async getTokenOtherChainZKLAmount(
    address: string,
    tokenAddress: string,
    network: string,
  ): Promise<string> {
    try {
      const options = {
        method: 'GET',
        headers: { 'x-api-key': this.chainBaseKey },
      };
      const balanceResponse = await fetch(
        `${this.chainBaseUrl}account/tokens?chain_id=${
          networkChainIdMap[network.toLowerCase()]
        }&contract_address=${tokenAddress}&address=${address}`,
        options,
      );
  ...
  private async getOtherChainNowBlock(network: string) {
    try {
      ...
      const transfersResponse = await fetch(
        `${this.chainBaseUrl}block/number/latest?chain_id=${
          networkChainIdMap[network.toLowerCase()]
        }`,
        options,
      );
  ...
  private async getOtherChainZKLTransfers(
    tokenAddress: string,
    ...
    try {
      ...
      do {
        ...
        const transfersResponse = await fetch(
          `${this.chainBaseUrl}token/transfers?chain_id=${
            networkChainIdMap[network.toLowerCase()]
          }&contract_address=${tokenAddress}&from_block=${fromBlockNumber}&to_block=${toBlockNumber}&page=${page}&limit=${limit}`,
          options,
        );
        const response = await transfersResponse.json();
        page = page + 1;
  ...
  private async sendMessageToLarkGroup(
    from: string,
    to: string,
    network: string,
    value: string,
          ...
          elements: [
            {
              tag: 'div',
              text: {
                content: `When a transfer ${txHash} value ${formatUnits(
                  value,
                )} is made from ${from} to ${to} in network ${network}, it triggers a high-amount alert.`,
                tag: 'plain_text',
              },
            },
          ],
